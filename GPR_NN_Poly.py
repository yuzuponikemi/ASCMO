# -*- coding: utf-8 -*-
"""
Created on Fri Dec 11 16:29:25 2020
...
# 元データの作成
n=160 #signal num
σ_noise = 0.5  # assumption noise ratio
noiseAct = 0.9 # actual noise ratio
missing_value_rate = 0.5 #sampling_rate
snratio = 10 #signal noise ratio
#n_outl = 40 #num of outliers
r_outl = 2 # ratio of outling
xwidth = 3
regr500 = MLPRegressor(activation='logistic',random_state=0,learning_rate_init=0.01,\
                       learning_rate='adaptive',alpha=0.001, \
                       hidden_layer_sizes=(100,200,200,), max_iter=5000)
...
print('σ_noise = '+str(σ_noise))

def datacreation(n, noiseAct, missing_value_rate, snratio, r_outl):
    data_x = np.linspace(0, xwidth*np.pi, n)
    data_x_org = np.linspace(0, xwidth*np.pi, 1000)
    ...
    sample_index = np.delete(sample_index, np.where((int(n-70) < sample_index)*(sample_index < int(n-30))))
    ...
    return data_x, data_x_org, data_y, data_y_org, sample_index, outl_y



def sampleplot(data_x, data_x_org, data_y, data_y_org, sample_index, outl_y):
...
def plotgpr(data_x, data_y, sample_index, outl_y, xtestnum=1000):
    #トレーニングデータ
    ...
    xtest =  np.linspace(0, xwidth*np.pi, xtestnum)  
    ...
    plt.plot(data_x_org, data_y_org,color='black', label='correct function',linestyle='dashed')
    # `np.polyfit` で、次式で近似
    plt.plot(xtest, np.poly1d(np.polyfit(xtrain, ytrain, 6))(xtest), label='Polyfit d=6', color="green")
    # `np.polyfit` で、次式で近似
    plt.plot(xtest, np.poly1d(np.polyfit(xtrain, ytrain, 12))(xtest), label='Polyfit d=12', color="orange")
    #NNnet
    regr500.fit(xtrain.reshape(-1,1), ytrain)
    plt.plot(xtest, regr500.predict(xtest.reshape(-1,1)), label='NN', color="purple")
#    regr1000 = MLPRegressor(random_state=1, max_iter=1000).fit(xtrain.reshape(-1,1), ytrain)
#    plt.plot(xtest, regr1000.predict(xtest.reshape(-1,1)), label='NN', color="yellow")
#    regr10000 = MLPRegressor(random_state=1, max_iter=10000).fit(xtrain.reshape(-1,1), ytrain)
#    plt.plot(xtest, regr10000.predict(xtest.reshape(-1,1)), label='NN', color="green")
    ## 元の信号
    #plt.plot(data_x, data_y, 'x', color='green', label='correct signal')
    #外れ値
    if wOutl == True:plt.plot(data_x[sample_index], outl_y, 'o', color='purple')#, label='outlier')
    ...
    filename = "./images/GPsignal_SN_"+str(snratio)+"_n_"+str(int(n*missing_value_rate))+"_x_"+str(int(xwidth))+"pi.png"
    plt.savefig(filename)
    
    plt.show()
   # print(regr500.predict(xtest.reshape(-1,1)))

# 平均値±(標準偏差×2) … 95.4%の確率で範囲内に指定の数値が現れる
    
    
data_x, data_x_org, data_y, data_y_org, sample_index, outl_y = datacreation(n, noiseAct, missing_value_rate, snratio, r_outl)

sampleplot(data_x, data_x_org, data_y, data_y_org, sample_index, outl_y)
plotgpr(data_x, data_y, sample_index, outl_y, xtestnum=1000)