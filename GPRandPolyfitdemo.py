# -*- coding: utf-8 -*-
"""
Created on Tue Dec  8 09:54:42 2020
...
# 元データの作成
n=400 #signal num
σ_noise = 0.9  # assumption noise ratio
...
def datacreation(n, noiseAct, missing_value_rate, snratio, r_outl):
    ...
    sample_index = np.delete(sample_index, np.where((int(n-200) < sample_index)*(sample_index < int(n-150))))
...
def plotgpr(data_x, data_y, sample_index, outl_y, xtestnum=1000):
    ...
    # `np.polyfit` で、2次式で近似
    plt.plot(xtest, np.poly1d(np.polyfit(xtrain, ytrain, 12))(xtest), label='Polyfit d=12', color="purple")
    ## 元の信号
    #plt.plot(data_x, data_y, 'x', color='green', label='correct signal')
    #外れ値
    if wOutl == True:plt.plot(data_x[sample_index], outl_y, 'o', color='purple', label='outlier')
